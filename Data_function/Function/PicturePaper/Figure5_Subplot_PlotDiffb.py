...
for i, c in zip(TIME,range(1,4,2)):
	...
	axes.set_position([chartBox.x0-0.055, chartBox.y0+0.04-p,
             	chartBox.width,
             	chartBox.height])
	DissolvedIron=loadmat('/home/natalia/Documentos/Tesis/Functions/Otros/DissolvedIron.mat')
	DissolvedIron=np.array([DissolvedIron['OUT'][:,:,pp]]) # Aquí es al revés, 0 es LGM y 1 Holoceno-->Ver: make_Solubility_escalado_region.m
	DissolvedIron=DissolvedIron[0,:,:]
	ii+=1
	zz=-1
	p+=0.04
	for j,d in zip(REGION,colors):
		...
		for k in SOL:
			...
			for h in MODEL:
				vv+=1
				Sol.append(DissolvedIron[zz,4*vv+ll])
				Sol2.append(DissolvedIron[zz,4*vv+ll])
	...
	if i=='LGM':
		...
		axes.set_xlabel('Median Iron input [umol Fe m-2 yr-1]',fontsize=9,fontweight='bold')
	else:
		plt.title('a)',x=-0.16, y=1.05)
	plt.savefig('/home/natalia/Documentos/DATA/TesisI/Data_function/Function/PicturePaper/Figure5_CO2Diff_DissolvedFe.png')
