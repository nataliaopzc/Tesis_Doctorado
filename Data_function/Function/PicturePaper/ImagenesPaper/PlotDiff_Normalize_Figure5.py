...
for i in TIME:
	...
	for j,d in zip(REGION,colors):
		...
		for k in SOL:
			...
			for xp,yp,m in zip(Sol,CO22,Simbolos):
				axes.grid(True)
				axes.scatter(xp, yp, marker=m,c=d)#axes.scatter(xp, yp,s=4**(ll+1.2), marker=m,c=d)
			plt.xlim(-0.1,8) #LGM
	...
	plt.savefig('/home/natalia/Documentos/DATA/TesisI/Data_function/Function/PicturePaper/Figure5_CO2Diff_Normalize_'+i+'.png')
plt.show()
