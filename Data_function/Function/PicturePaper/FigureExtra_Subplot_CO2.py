...
p=0
for i, c in zip(TIME,range(1,4,1)):
	axes = figure.add_subplot(1,2,c)
	chartBox = axes.get_position()
	axes.set_position([chartBox.x0, chartBox.y0,
             	chartBox.width,
             	chartBox.height])
	p+=0.04
	ii=-1
	...
	for k in SOL:
		...
		for yp,m in zip(CO2,Simbolos):
			...
			axes.scatter(index2[ll], yp,s=16, marker=m,c="k")
		axes.set_xlim(-1.5, 13.5)
		axes.set_ylim(200, 350)
		axes.set_xticks([0,3, 6,9,12])
 	#axes.set_yticks(np.arange(-7.5,15,2.5))
		axes.set_xticklabels(['0.3', '0.6', '1','2', '3'],fontsize=8)
		plt.yticks(fontsize=8) 
		axes.set_ylabel(r'$ \bf $' 'pCO' r'$\bf _2$' ' [ppm]',fontsize=9,fontweight='bold')
		plt.axvspan(-1.5, 1.5, facecolor='gray', alpha=0.3) #	plt.axhline(y=0,linestyle = '-')
		plt.axvspan(4.5,7.5, facecolor='gray', alpha=0.3)
		plt.axvspan(10.5,13.5, facecolor='gray', alpha=0.3)
		plt.tick_params(axis='x', bottom = False)
 	#plt.xlim(-2,22)
		if i=='LGM':
			axes.set_xlabel('Multiplication factor of iron solubility',fontsize=9,fontweight='bold')
			plt.title('Last Glacial Maximum')
		else:
			axes.set_xlabel('Multiplication factor of iron solubility',fontsize=9,fontweight='bold')
			plt.title('Holocene')
plt.savefig('/home/natalia/Documentos/DATA/TesisI/Data_function/Function/PicturePaper/ExtraFigure_Subplot_CO2.png')
plt.show()
