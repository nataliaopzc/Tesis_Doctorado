...
bplot=plt.boxplot(dCO2,showfliers=False,patch_artist=True, vert=False)
...
for h in range(1,22):
    for m,l in zip(Sim,range(7)):
        plt.scatter(dCO2[l,h-1],h,s=16,marker=m,c='black')

for i,j in zip(np.arange(1.5,21,8), np.arange(5.5,22,8)):
	plt.axhspan(i,j,facecolor='gray', alpha=0.3) 
plt.yticks([1, 3.5, 7.5,11.5,15.5,19.5],['Global', 'NA', 'NP', 'CEP','SP','SAI'],fontsize=9,fontweight='bold')
plt.xlabel(r'$ \bf \Delta$' 'pCO' r'$\bf _2$' ' [ppm]',color='black',fontsize=9, fontweight='bold')
plt.ylim(0.5, 21.5)
plt.xlim(0, 50)
plt.grid(axis='x',linestyle = '--')
plt.legend(handles=legend_elements, loc='upper right',fontsize=9, bbox_to_anchor=(1.25, 1.03))
plt.tick_params(axis='y', left = False)
plt.savefig('/home/natalia/Documentos/DATA/TesisI/Data_function/Function/PicturePaper/Prueba_Figure6_Boxplot_Tabla.png')